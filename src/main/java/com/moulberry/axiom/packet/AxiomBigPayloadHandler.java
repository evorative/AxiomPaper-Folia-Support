...
public class AxiomBigPayloadHandler extends MessageToMessageDecoder<ByteBuf> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        try {
            ...
            if (packetId == this.payloadId) {
                identifier = Utf8String.read(in, 32767);
                ...
                if (handler != null) {
                    ...
                    if (handler.handleAsync()) {
                        ...
                        final String packetIdentifier = identifier;
                        ...
                        Bukkit.getGlobalRegionScheduler().execute(AxiomPaper.PLUGIN, () -> {
                            try {
                                ...
                                callReceive(handler, player, friendlyByteBuf, packetIdentifier);
                            } catch (Throwable t) {
                                AxiomPaper.PLUGIN.getLogger().log(Level.SEVERE, "Error while processing Axiom packet " + packetIdentifier, t);
                                player.connection.disconnectAsync(net.minecraft.network.chat.Component.literal("Error while processing Axiom packet " + packetIdentifier + ": " + t), DisconnectionReason.UNKNOWN);
                            }
                        });
                    }
                    success = true;
            ...
            if (!(t instanceof IndexOutOfBoundsException && allowIndexOutOfBounds)) {
                AxiomPaper.PLUGIN.getLogger().log(
                    Level.SEVERE,
                    "Error while decoding Axiom packet for player " + player.getBukkitEntity().getName() +
                        " (" + player.getUUID() + "), payloadId=" + this.payloadId +
                        ", packetId=" + (packetId == Integer.MIN_VALUE ? "<unread>" : Integer.toString(packetId)) +
                        ", identifier=" + (identifier == null ? "<unread>" : identifier),
                    t
                );
    ...
    private static void callReceive(PacketHandler handler, ServerPlayer player, RegistryFriendlyByteBuf friendlyByteBuf, String identifier) {
        ...
        try {
            ...
            player.connection.disconnectAsync(net.minecraft.network.chat.Component.literal("Error while processing Axiom packet " + identifier + ": " + reason), DisconnectionReason.UNKNOWN);
        }
    }
