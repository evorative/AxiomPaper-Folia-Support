...
public class SetTimePacketListener implements PacketHandler {
    ...
    public void onReceive(Player player, RegistryFriendlyByteBuf friendlyByteBuf) {
        ...
        // Change time on the global region, which Folia requires for world time / gamerule updates.
        final Integer finalTime = time;
        final Boolean finalFreezeTime = freezeTime;
        Bukkit.getGlobalRegionScheduler().execute(this.plugin, () -> {
            try {
                if (finalTime != null) {
                    player.getWorld().setTime(finalTime);
                }
                if (finalFreezeTime != null) {
                    level.getGameRules().set(GameRules.ADVANCE_TIME, !finalFreezeTime, null);
                }
            } catch (Throwable t) {
                this.plugin.getLogger().log(java.util.logging.Level.SEVERE,
                    "Failed to apply world time changes for player " + player.getName() + " (" + player.getUniqueId() + ")",
                    t);
                player.kick(Component.text("Error while processing packet axiom:set_world_time: " + t));
            }
        });
    }
