...
public class BiomeBuffer {
    ...
    private int getPaletteIndex(ResourceKey<Biome> biome) {
        ...
        if (index >= this.palette.length) {
            throw new IllegalArgumentException("Biome buffer palette exceeded " + this.palette.length + " entries. " +
                "This selection contains too many distinct biomes for the current packet format.");
        }
